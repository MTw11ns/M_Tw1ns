                  ...
                  itemBuilder: (context, index) => Consumer<DataProvider>(
                      builder: (context, radiodata, Widget? child) {
                        return radiolistile(
                            ...
                            isfav: radiodata.allMusicItems[index].isFav! == "true" ? true : false,
                          ...
                          ontap: () {
                            ...
                            // ontap: () {
                            //   setState(() {
                            //     DataProvider.title = radiodata.allMusicItems[index].name!;
                            //     DataProvider.category = radiodata.allMusicItems[index].type!;
                            //     DataProvider.image = radiodata.allMusicItems[index].imageUrl!;
                            //     DataProvider.soundUrl = radiodata.allMusicItems[index].soundUrl!;
                            //     DataProvider.playPauseIcon = Icons.pause;
                            //     DataProvider.id = radiodata.allMusicItems[index].id!;
                            //     Provider.of<DataProvider>(context, listen: false)
                            //         .getAudio(radiodata.allMusicItems[index].soundUrl!);
                            //   });
                            // },
                        );
                      }),
                  separatorBuilder: (context, index) => SizedBox(height: 10),
                  itemCount: Provider.of<DataProvider>(context).allMusicItems.length),
            ),
            InkWell(
              onTap: () {
                setState(() {
